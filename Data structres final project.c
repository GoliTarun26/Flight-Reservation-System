...
void bookSeat(struct Flight* root, char flightNumber[], char name[]) {
    ...
    if (flight == NULL) {
        ...
        if (flight->booked < flight->capacity) {
            ...
            printf("Seat booked successfully on flight %s to %s for %s.\n", flight->flightNumber, flight->destination, name);
        } else {
            printf("Sorry, the flight %s to %s is fully booked.\n", flight->flightNumber, flight->destination);
        }
    }
}
...
int main() {
    ...
    root = insertFlight(root, "ABC123", "New York", 100);
    root = insertFlight(root, "XYZ456", "Los Angeles", 150);
    root = insertFlight(root, "DEF789", "Chicago", 120);
